...
class CharacterGenerator {
  ...
  static List<String> _filterGender(List<String> paths, String gender) {
    final filtered = paths
        .where((p) => _filename(p).startsWith(gender))
        .toList();
  ...
  static Future<Character> generate() async {
    ...
    final heads = _filterGender(
      assets['heads']!,
      gender,
    ).where((p) => _headColor(p) == color).toList();
    final faces = _filterGender(
      assets['faces']!,
      gender,
    ).where((p) => _faceColor(p, gender) == color).toList();
